...
type ProfileEditPageProps = Partial<UserProfileData> & {
	backButton?: string
	saveButton?: string
	loading?: boolean
	[key: string]: unknown
		...
		super('main', {
			...
			events: {
				click: async (e: Event) => {
					...
					if (target.closest('.avatar-overlay')) {
						const input = this.getContent()?.querySelector(
							'input.avatar-input[name="avatar"]',
				...
				change: async (e: Event) => {
					...
					if (!target.matches('input.avatar-input[name="avatar"]')) return
				...
				submit: async (e: Event) => {
					...
					if (form.id !== 'edit-profile-form') return
